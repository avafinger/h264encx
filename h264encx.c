...
struct h264enc_internal {
	...
	struct h264enc_ref_pic {
        struct ve_mem *luma_buffer_ionmem;
        uint32_t chroma_buffer_phys;
		struct ve_mem *extra_buffer_ionmem; /* unknown purpose, looks like smaller luma */
		void *luma_buffer, *chroma_buffer;
		void *extra_buffer; /* unknown purpose, looks like smaller luma */
	} ref_picture[2];

    struct ve_mem *extra_buffer_line_ionmem, *extra_buffer_frame_ionmem; /* unknown purpose */
...
void h264enc_free(h264enc *c)
{
	...
	ve_free(c->extra_buffer_line_ionmem);
	ve_free(c->extra_buffer_frame_ionmem);
	for (i = 0; i < 2; i++)
	{
		ve_free(c->ref_picture[i].luma_buffer_ionmem);
		ve_free(c->ref_picture[i].extra_buffer_ionmem);
	}
	ve_free(c->bytestream_buffer_ionmem);
	ve_free(c->luma_buffer_ionmem);
	free(c);
}
...
h264enc *h264enc_new(const struct h264enc_params *p)
{
    ...
    c->luma_buffer_ionmem = ve_malloc(c->input_buffer_size);
    if (c->luma_buffer_ionmem == NULL)
		goto nomem;
	c->luma_buffer = c->luma_buffer_ionmem->virt;
	if (c->luma_buffer == NULL)
		goto nomem;

	c->chroma_buffer = c->luma_buffer + p->src_width * p->src_height;
    c->chroma_buffer_phys = ve_virt2phys(c->luma_buffer_ionmem) + p->src_width * p->src_height;

	/* allocate bytestream output buffer */
	c->bytestream_buffer_size = 1 * 1024 * 1024;
    c->bytestream_buffer_ionmem = ve_malloc(c->bytestream_buffer_size);
    if (c->bytestream_buffer_ionmem == NULL)
		goto nomem;
	c->bytestream_buffer = c->bytestream_buffer_ionmem->virt;
	...
	for (i = 0; i < 2; i++)
	{
        c->ref_picture[i].luma_buffer_ionmem = ve_malloc(luma_size + chroma_size);
        if (c->ref_picture[i].luma_buffer_ionmem == NULL)
			goto nomem;
		c->ref_picture[i].luma_buffer = c->ref_picture[i].luma_buffer_ionmem->virt;
		c->ref_picture[i].chroma_buffer = c->ref_picture[i].luma_buffer + luma_size;
        c->ref_picture[i].chroma_buffer_phys = ve_virt2phys(c->ref_picture[i].luma_buffer_ionmem) + luma_size;
		c->ref_picture[i].extra_buffer_ionmem = ve_malloc(luma_size / 4);
        if (c->ref_picture[i].luma_buffer_ionmem == NULL)
			goto nomem;
        c->ref_picture[i].extra_buffer = c->ref_picture[i].luma_buffer_ionmem->virt;
		if (c->ref_picture[i].luma_buffer == NULL || c->ref_picture[i].extra_buffer == NULL)
			goto nomem;
	}

	/* allocate unknown purpose buffers */
    c->extra_buffer_frame_ionmem = ve_malloc(ALIGN(c->mb_width, 4) * c->mb_height * 8);
	c->extra_buffer_line_ionmem = ve_malloc(c->mb_width * 32);
    if (c->extra_buffer_frame_ionmem == NULL || c->extra_buffer_line_ionmem == NULL)
        goto nomem;
  	c->extra_buffer_frame = c->extra_buffer_frame_ionmem->virt;
    c->extra_buffer_line = c->extra_buffer_line_ionmem->virt;
...
int h264enc_encode_picture(h264enc *c)
{
    ...
    ve_flush_cache(c->bytestream_buffer_ionmem);
	// ve_flush_cache(c->luma_buffer, c->input_buffer_size);
    ve_flush_cache(c->luma_buffer_ionmem);

	/* set output buffer */
	writel(0x0, c->regs + VE_AVC_VLE_OFFSET);
	writel(ve_virt2phys(c->bytestream_buffer_ionmem), c->regs + VE_AVC_VLE_ADDR);
	writel(ve_virt2phys(c->bytestream_buffer_ionmem) + c->bytestream_buffer_size - 1, c->regs + VE_AVC_VLE_END);
	...
	/* set input buffer */
	writel(ve_virt2phys(c->luma_buffer_ionmem), c->regs + VE_ISP_INPUT_LUMA);
	writel(c->chroma_buffer_phys, c->regs + VE_ISP_INPUT_CHROMA);

	/* set reconstruction buffers */
	struct h264enc_ref_pic *ref_pic = &c->ref_picture[c->current_frame_num % 2];
	writel(ve_virt2phys(ref_pic->luma_buffer_ionmem), c->regs + VE_AVC_REC_LUMA);
	writel(ref_pic->chroma_buffer_phys, c->regs + VE_AVC_REC_CHROMA);
	writel(ve_virt2phys(ref_pic->extra_buffer_ionmem), c->regs + VE_AVC_REC_SLUMA);
	...
	if (c->current_slice_type != SLICE_I)
	{
		...
		writel(ref_pic->chroma_buffer_phys, c->regs + VE_AVC_REF_CHROMA);
		writel(ve_virt2phys(ref_pic->extra_buffer_ionmem), c->regs + VE_AVC_REF_SLUMA);
	}

	/* set unknown purpose buffers */
	writel(ve_virt2phys(c->extra_buffer_line_ionmem), c->regs + VE_AVC_MB_INFO);
	writel(ve_virt2phys(c->extra_buffer_frame_ionmem), c->regs + VE_AVC_UNK_BUF);
